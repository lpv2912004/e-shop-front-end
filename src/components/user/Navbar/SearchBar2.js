...
import { FaSearch, FaMicrophone } from 'react-icons/fa'; // Import thêm biểu tượng Microphone
import axios from "axios";
import { useHistory } from "react-router-dom"; // Import useHistory for React Router v5
import SpeechRecognition, { useSpeechRecognition } from 'react-speech-recognition'; // Import thư viện giọng nói
...
const SearchBar = () => {
    ...
    // Hook cho nhận diện giọng nói
    const { transcript, listening, resetTranscript } = useSpeechRecognition();

    useEffect(() => {
        setKeyword(transcript); // Cập nhật từ khóa tìm kiếm bằng giọng nói
    }, [transcript]);
    useEffect(() => {
